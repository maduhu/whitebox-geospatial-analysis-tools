...
public abstract class KdTree<T> {
    ...
    public void addPoint(float[] location, T value) {
        ...
        for (int a = 0; a < location.length; a++) {
            doubleLocation[a] = location[a];
        }

        addPoint(doubleLocation, value);
    ...
    public List<Entry<T>> neighborsWithinRange(float[] location, double range) {
        double[] doubleLocation = new double[location.length];
        for (int a = 0; a < location.length; a++) {
            doubleLocation[a] = location[a];
        }
        return neighborsWithinRange(doubleLocation, range);
    }
    ...
    public List<Entry<T>> nearestNeighbor(float[] location, int count, boolean sequentialSorting) {
        ...
        return nearestNeighbor(doubleLocation, count, sequentialSorting);
    }
    ...
    @SuppressWarnings("unchecked")
    public List<Entry<T>> nearestNeighbor(double[] location, int count, boolean sequentialSorting, boolean useQuadBasedSearch) {
        if (!useQuadBasedSearch) {
            return nearestNeighbor(location, count, sequentialSorting);
        } else {
            ArrayList<Entry<T>> results = new ArrayList();
            for (int q = 1; q < 5; q++) {
                List<Entry<T>> quadResults = nearestNeighbor(location, count / 4, (byte)q, sequentialSorting);
                for (Entry<T> entry : quadResults) {
                    results.add(entry);
                }
            }
            return results;
    ...
    @SuppressWarnings("unchecked")
    public List<Entry<T>> nearestNeighbor(double[] location, int count, byte quadrant, boolean sequentialSorting) {
        KdTree<T> cursor = this;
        cursor.status = Status.NONE;
        double range = Double.POSITIVE_INFINITY;
        ResultHeap resultHeap = new ResultHeap(count);
            ...
            if (cursor.status == Status.NONE && cursor.locations != null) {
                ...
                if (cursor.locationCount > 0) {
                    if (cursor.singularity) {
                        ...
                        if (dist <= range) {
                            for (int i = 0; i < cursor.locationCount; i++) {
                                if (getQuadrant(location, cursor.locations[i]) == quadrant) {
                                    resultHeap.addValue(dist, cursor.data[i]);
                                }
                            }
                        }
    ...
    private int getQuadrant(double[] location1, double[] location2) {
        double val1 = location2[0] - location1[0]; // difference in x
        double val2 = location2[1] - location1[1]; // difference in y
