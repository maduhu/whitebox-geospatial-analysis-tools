...
def name = "VectorFeatureDensity"
def descriptiveName = "Vector Feature Density"
def description = "Calculates a type of heat map or density surface."
def toolboxes = ["VectorTools"]

public class VectorFeatureDensity implements ActionListener {
...
//            di.addPropertyChangeListener(lstr1)
	...
	private void execute(String[] args) {
		try {
            ...
            while (stRow < (rows - 1) || endingRow < (rows - 1)) {
				...
				if (stRow < rows) {
					tasks.add(new DoWorkOnRow(input, stRow, nRows, 
    ...
    class DoWorkOnRow implements Callable<RowNumberAndData> {
	    ...
	    DoWorkOnRow(ShapeFile input, int startingRow, int nRows, 
			...
			for (int row in 0..<nRows) {
				...
				for (int col in 0..<cols) {
			        ...
			        if (results.size() > 0) {
			        	...
			        	retData[row][col] = (double)(hs.size() / area)
			        } else {
...
if (args == null) {
	...
	def myClass = new VectorFeatureDensity(pluginHost, args, name, descriptiveName)
}
