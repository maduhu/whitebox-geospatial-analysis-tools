...
def name = "FurtherestUpstreamChannelHeadDistance"
def descriptiveName = "Furtherest Upstream Channel Head Distance"
def description = "Calculates the distance to the furthest upstream channel head."
def toolboxes = ["StreamAnalysis"]

public class FurtherestUpstreamChannelHeadDistance implements ActionListener {
	private WhiteboxPluginHost pluginHost
	private ScriptDialog sd;
	private String descriptiveName
	
	public FurtherestUpstreamChannelHeadDistance(WhiteboxPluginHost pluginHost, 
	...
	private void execute(String[] args) {
		try {
			...
			double flowDir, nFlowDir, streamVal, length, order, id
			...
			WhiteboxRaster output2 = new WhiteboxRaster(outputFile2, "rw", 
  		  	     streamsFile, DataType.FLOAT, outNodata, outNodata)
  		  	output2.setDataScale(DataScale.CATEGORICAL)
  		  	output2.setPreferredPalette("qual.plt")
  		  	output2.isTemporaryFile = true

//  		  	String outputFile3 = outputFile.replace(".dep", "_HackSO.dep")
//			WhiteboxRaster output3 = new WhiteboxRaster(outputFile3, "rw", 
//  		  	     streamsFile, DataType.FLOAT, outNodata, outNodata)
//  		  	output3.setDataScale(DataScale.CATEGORICAL)
//  		  	output3.setPreferredPalette("qual.plt")
  		  	
  		  	//NibbleArray2D numInflowing = new NibbleArray2D(rows, cols)

  		  	Queue<KeyChannelCell> heads = new LinkedList<>()
  		  	Queue<KeyChannelCell> confluences = new LinkedList<>()
				...
				for (int col in 0..(cols - 1)) {
					streamVal = streams.getValue(row, col)
  					if (streamVal != nodata && streamVal > 0) {
                        ...
                        if (i == 0) {
                        	heads.add(new KeyChannelCell(row, col, id))
                        	id++
                        }
                        ...
                        if (isOutlet) {
                        	confluences.add(new KeyChannelCell(row, col, 1))
                        }
			...
			while (heads.size() > 0) {
				KeyChannelCell head = heads.poll()
				x = head.column
                y = head.row
                id = head.id
                ...
                while (flag) {
                    ...
                    if (flowDir != pointerNodata && flowDir > 0) {
                        ...
                        streamVal = streams.getValue(y, x)
  						if (streamVal != nodata && streamVal > 0) {
  							if (length > output.getValue(y, x)) {
  								output.setValue(y, x, length)
  								output2.setValue(y, x, id)
  							} else {
  								flag = false
  							}
...
//				KeyChannelCell confluence = confluences.poll()
//				order = confluence.id
//				x = confluence.column
//                y = confluence.row
...
//                        		confluences.add(new KeyChannelCell(y + dY[c], x + dX[c], order + 1))
//                        	}
//                        }
//                        
//                    	x = nx
...
//			}
			...
			output2.addMetadataEntry("Created by the "
	                    + descriptiveName + " tool.")
	        output2.addMetadataEntry("Created on " + new Date())
			output2.close()

//			output3.addMetadataEntry("Created by the "
//	                    + descriptiveName + " tool.")
//	        output3.addMetadataEntry("Created on " + new Date())
//			output3.close()
	
			// display the output image
			//pluginHost.returnData(outputFile3)
			//pluginHost.returnData(outputFile2)
    ...
    class KeyChannelCell {
		int row
		int column
		double id
		
		public KeyChannelCell(int row, int column, double id) {
			this.row = row
			this.column = column
			this.id = id
		}
...
if (args == null) {
	...
	def tdf = new FurtherestUpstreamChannelHeadDistance(pluginHost, args, name, descriptiveName)
}
