...
def name = "CreateHexagonalVectorGrid"
def descriptiveName = "Create Hexagonal Vector Grid"
def description = "Creates a grid of hexagonal vector polygons"
def toolboxes = ["VectorTools"]

public class CreateHexagonalVectorGrid implements ActionListener {
    private WhiteboxPluginHost pluginHost
    private ScriptDialog sd;
    private String descriptiveName
	
    public CreateHexagonalVectorGrid(WhiteboxPluginHost pluginHost, 
        ...
        if (args.length > 0) {
            ...
            def helpFile = "CreateHexagonalVectorGrid"
            sd.setHelpFile(helpFile)
		
            // Specifying the source file allows the 'view code' 
            // button on the tool dialog to be displayed.
            def pathSep = File.separator
            def scriptFile = pluginHost.getResourcesDirectory() + "plugins" + pathSep + "Scripts" + pathSep + "CreateHexagonalVectorGrid.groovy"
    ...
    private void execute(String[] args) {
        try {
            ...
            if (orientation.equals("h")) { // horizontal orientation
				...
				if (rows * cols > 100000) {
					int ret = pluginHost.showFeedback("This operation is going to produce a vector file with a very large number of polygons which may be difficult to handle. Do you want to proceed?", 1, 1)
					if (ret == 1) {
						return
					}
				}
...
if (args == null) {
    ...
    def f = new CreateHexagonalVectorGrid(pluginHost, args, descriptiveName)
}
