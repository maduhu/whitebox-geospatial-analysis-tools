...
def name = "LowestPosition"
def descriptiveName = "Lowest Position"
def description = "Identifies the stack position of the min value within a raster stack on a cell-by-cell basis"
def toolboxes = ["OverlayTools"]

public class LowestPosition implements ActionListener {
	private WhiteboxPluginHost pluginHost
	private ScriptDialog sd
	private String descriptiveName
	private String name
	
	public LowestPosition(WhiteboxPluginHost pluginHost, 
		...
		try {
			...
			for (int row = 0; row < rows; row++) {
				...
				for (int col = 0; col < cols; col++) {
					...
					if (value != Double.POSITIVE_INFINITY && !nodataOnStack) {
						outData[col] = index;
					} else {
						outData[col] = nodata;
					}
...
if (args == null) {
	...
	def f = new LowestPosition(pluginHost, args, name, descriptiveName)
}
